...
// pointer-driven gesture recognizers built bottom-up.
// primitives (trackSingle, trackMulti) handle pointer tracking & lifecycle.
// preset recognizers build on top of these primitives.
...
// :::::: CORE PRIMITIVES (EXPORTED LOW-LEVEL TRACKERS)

// low-level tracker for single-pointer interactions. handles primary pointer verification,
// pointer capture management, lifecycle events, and state resetting.
export function trackSingle ({ onDown, onMove, onUp, onCancel }) {
  let id = null;
...
// low-level tracker for multi-pointer interactions (N fingers). manages pointer map state,
// active gesture lifecycle, and automatic default prevention during moves.
export function trackMulti ({ count = 2, onStart, onMove, onEnd }) {
...
// hands back both the raw matrix and its decomposition.
...
// :::::: PRESET RECOGNIZERS (BUILT ON TOP OF PRIMITIVES)

// tap, double-tap and long-press built on trackSingle.
...
// press-and-repeat button built on trackSingle.
export function holdable ({ onHold, delay = 500, speed = 100 } = {}) {
  ...
  return {
    ...
    style   : NO_SELECT,
    destroy : () => { stop(); tracker.reset(); }
  };
}

// directional flick built on trackSingle.
...
// single-pointer drag built on trackSingle.
...
// two-finger resize built on trackMulti.
...
// normalized wheel input handler.
...
// two-finger pinch built on trackMulti.
...
// two-finger rotation built on trackMulti.
...
// free move + scale + rotate (accumulates similarity matrix over 1 or 2 pointers).
...
// gestures(element, options) — binds active presets to a DOM element and resolves CSS touchAction conflicts.
